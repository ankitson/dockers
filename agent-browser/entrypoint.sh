...
def rewrite_host(header_bytes, new_host):
    """Replace the Host header so chrome (which only accepts localhost/IP) is happy
    when clients reach the relay via the container hostname (openclaw-browser).
    OpenClaw normalizes the discovered webSocketDebuggerUrl back to the original
    discovery host, so this rewrite is transparent to the client."""
    try:
        text = header_bytes.decode("iso-8859-1")
    except UnicodeDecodeError:
        return header_bytes
    ...
    for i, line in enumerate(lines):
        if i == 0:
            out.append(line)
            continue
        name, s, _ = line.partition(":")
        if s and name.strip().lower() == "host":
            out.append("Host: " + new_host)
            replaced = True
        else:
            out.append(line)
    if not replaced:
        out.insert(1, "Host: " + new_host)
    return ("\r\n".join(out) + sep + body).encode("iso-8859-1")
...
class Handler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        upstream.sendall(rewrite_host(header_bytes, "localhost:" + str(UPSTREAM_PORT)))
